    ...
    const unsigned cols)
{
  ...
  const unsigned char val = max(
      (unsigned char)(round(val_ratio * (ma - mi)) + mi), (unsigned char)1);
  for (unsigned r = 0; r < rows; ++r) {
    for (unsigned c = 0; c < cols; ++c) {
      if (si[r][c] < val)
        continue;
    ...
    const unsigned cols)
{
  ...
  typedef const unsigned char ST[rows][cols];
  auto& si = *(ST*)s;
  const unsigned len = 4 * (rows + cols) - 7,
                 dl[8] = { cols, rows, rows, cols, cols, rows, rows, cols };
  vector<unsigned> sum(len, 0), sumn(len, 0);
  unsigned *p[8], *pn[8];
  float* res[8];
  ...
  res[0][0] = pn[0][0] > 0 ? (double)p[0][0] / pn[0][0] : 0;
  for (unsigned d = 0; d < 8; ++d) {
    for (unsigned l = 1; l < dl[d]; ++l) {
      res[d][l] = pn[d][l] > 0 ? (double)p[d][l] / pn[d][l] : 0;
    }
    ...
    const unsigned cols)
{
  ...
  res[0][0] = pn[0][0] > 0 ? (double)p[0][0] / pn[0][0] / ma : 0;
  for (unsigned d = 0; d < 8; ++d) {
    for (unsigned l = 1; l < dl[d]; ++l) {
      res[d][l] = pn[d][l] > 0 ? (double)p[d][l] / pn[d][l] / ma : 0;
    }
  }
  return 0;
}
    ...
    const unsigned cols)
{
  ...
  typedef const unsigned char ST[rows][cols];
  auto& si = *(ST*)s;
  const unsigned len = 4 * (rows + cols - 2), rows1 = rows - 1,
                 cols1 = cols - 1,
                 dl[8]
      = { cols1, rows1, rows1, cols1, cols1, rows1, rows1, cols1 };
  vector<unsigned> sum(len, 0), sumn(len, 0);
  unsigned *p[8], *pn[8];
  float* res[8];
  p[0] = &sum[0] - 1, pn[0] = &sumn[0] - 1;
  res[0] = t - 1;
  for (unsigned d = 0; d < 7; ++d) {
    p[d + 1] = p[d] + dl[d] - 1;
    pn[d + 1] = pn[d] + dl[d] - 1;
